...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "scan_emails") {
        console.log("📥 Scan request received for mode:", request.mode);

        executeScan(request.days, request.mode, sendResponse);

        return true;
    }
});

async function executeScan(days, mode, sendResponse) {
    try {
        const results = await handleFullScan(days, mode);
        sendResponse({ emails: results });
    } catch (err) {
...
async function handleFullScan(days, mode) {
